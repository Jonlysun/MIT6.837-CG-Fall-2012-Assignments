...
void drawScene(void)
{
    ...
    GLfloat Lt0pos[] = {light_lp, light_rp, 5.0f, 1.0f };
...
void loadInput()
{
    ...
    // cin.getline(buffer, MAXIMUM_REPARSE_DATA_BUFFER_SIZE);
    ...
    while (true)
    {
        ...
        if (s=="v") {
            ...
            // a/b/v d/e/f g/h/i
            vector<int> f;
            vector<string> f_strs(3);
            ss >> f_strs[0] >> f_strs[1] >> f_strs[2];
            for (int i = 0; i < 3; i++) {
                char c[1024];
                strcpy(c, f_strs[i].c_str());         
                char* item = strtok(c, "/");
                while (item) {
                    f.push_back(atoi(item));               
                    item = strtok(NULL, "/");
                }
            }
            vecf.push_back(f);
